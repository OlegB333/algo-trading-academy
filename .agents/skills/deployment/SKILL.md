...
## Часть 1: VPS dry-run (Крипто / Binance)
...
## Часть 3: IB на удалённом VPS (Акции / Форекс / Фьючерсы)

На локальной машине Freqtrade подключается к **TWS** (Trader Workstation), который
ученик запускает сам. На удалённом VPS TWS запустить нельзя — нужен **IB Gateway**
в Docker-контейнере (`ghcr.io/gnzsnz/ib-gateway:stable`).

### 3.1 Настройка .env на VPS
...
### 3.2 Запуск IB-модуля на VPS

```bash
docker compose -f docker-compose.ib.yml up -d
```

Контейнер `ib-gateway` автоматически залогинится с указанными кредами
и будет держать сессию с IB-серверами 24/7.

### 3.3 Проверка

```bash
docker compose -f docker-compose.ib.yml ps
# ib-gateway должен показать: Up (healthy)
```

Логи gateway:
```bash
docker compose -f docker-compose.ib.yml logs ib-gateway --tail 20
```

### 3.4 Ограничения IB Gateway на VPS

- IB разрешает **одну активную сессию на аккаунт** — если на локальной машине
  открыт TWS с тем же аккаунтом, Gateway отвалится.
- Для VPS рекомендуется использовать **отдельный Paper/Live аккаунт** или
  закрывать локальный TWS когда бот работает на сервере.
- VNC для дебага gateway доступен по SSH-туннелю:
  ```bash
  ssh -L 5900:localhost:5900 user@YOUR_SERVER_IP
  # Затем VNC-клиент на localhost:5900, пароль: algotrading
  ```
...
6. **TWS открыт локально + Gateway на VPS с одним аккаунтом** → конфликт сессий IB
