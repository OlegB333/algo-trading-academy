...
2. Проверить папку `/freqtrade/user_data/config/` на наличие файла для фьючерсов (например, `config_futures.json`). Если его нет — **создай его сам** на базе `config.json`, изменив `"trading_mode"` на `"futures"` и добавив `"margin_mode": "isolated"`.
