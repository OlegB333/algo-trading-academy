...
- **Данные:** чем больше, тем надёжнее результат. Минимальный объём зависит от таймфрейма:
...
- **Фоновый бот остановлен:** `docker compose down` перед запуском.
  Hyperopt — это разовая команда (`docker compose run`), она не может работать
  одновременно с фоновым ботом (`docker compose up -d`).
...
5. **Запуск при работающем боте** → `docker compose down` перед hyperopt
