...
- **Рекомендуется остановить фоновый бот** (`docker compose down`) на слабых машинах.
  Hyperopt сильно нагружает CPU — параллельный бот может замедлить процесс.
...
```bash
# На слабых машинах лучше остановить фоновый бот
# docker compose down
...
5. **Hyperopt тормозит** → останови фоновый бот (`docker compose down`) для освобождения ресурсов
