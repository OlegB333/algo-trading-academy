...
Hyperopt выведет лучшие параметры. **Покажи результаты ученику таблицей и объясни что каждый параметр значит.** Не применяй результаты автоматически — жди решения ученика.

Объясни ученику правильный порядок:
1. Изучить найденные параметры — понятны ли они?
2. Обновить `default` значения в стратегии
3. Прогнать бэктест на **out-of-sample** периоде (те 30%, которые не использовались)
4. Если результат стабильный — сохранить

```python
# Пример: hyperopt нашёл оптимальный RSI = 25
# До:
buy_rsi = IntParameter(low=10, high=40, default=30, space="buy", optimize=True)
# После (ученик решил применить):
