...
Если файла `.env` **нет**, создай его самостоятельно:
```bash
cp .env.example .env
```
**Никогда не проси ученика выполнять консольные команды по созданию `cp .env.example .env` или запуску `docker compose ...` — делай это сам!**

Если файл есть (или ты его только что создал), но логин и пароль не заполнены, остановись и напиши ученику:
> ⚠️ Я подготовил файл `.env` в корне проекта. Для работы шлюза Interactive Brokers требуются данные от вашего аккаунта https://www.interactivebrokers.co.uk/
> Пожалуйста, откройте файл `.env`, впишите ваши логин (`TWS_USERID`) и пароль (`TWS_PASSWORD`), сохраните файл и напишите мне **«Готово»**. После этого я сам запущу шлюз.
