...
Если ученик просит создать стратегию для **фьючерсов**, ты должен сделать две вещи:
1. В самой стратегии установить `can_short = True`.
2. Проверить папку `/freqtrade/user_data/config/` на наличие файла для фьючерсов (например, `config_futures.json`). Если его нет — **создай его сам** на базе `config.json`, выполнив три замены:
   - Установи `"trading_mode": "futures"`.
   - Вставь `"margin_mode": "isolated"` (или `cross`).
   - Измени все названия пар в `pair_whitelist` на фьючерсный стандарт с `:USDT` на конце (например, `BTC/USDT` → `BTC/USDT:USDT`).
