...
**ПРИМЕЧАНИЕ ОБ ИНТЕГРАЦИИ:** Перед скачиванием любых данных с Interactive Brokers убедись, что TWS запущен и API-порт 7497 доступен (по навыку `ib-setup`). Если скачивание падает с ошибкой `Connection Refused` — подскажи ученику: *"TWS не запущен или API не включён. Запустите TWS и включите API (порт 7497)."*
...
> **НЕ НУЖНО** редактировать `futures_contracts`, `pair_whitelist` или любую другую часть конфига для скачивания данных. Плагин IB автоматически регистрирует любой тикер из таблицы выше. Просто передай пару через `--pairs`.
